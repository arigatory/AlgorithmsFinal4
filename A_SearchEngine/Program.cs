 ...
 * Сложность растет, если в разных документах будет встречаться больше уникальных слов.
 * Допустим, в документе1 будет k1 уникальных слов и тд, тогда k1+k2+...+kj > n уникальных слов. (n - всего уникальных слов) 
 * Для каждого слова мы заводим хеш таблицу, в которой храним документы, в которых это слово встречается, а также количество вхождений.
 * Это дополнительно от 1 до j элементов. То есть 2 граничных случая:
 * 1) все документы имеют уникальные слова -> O(n), так как дополнительно мы хранить будем ссылку лишь на один документ, 
 *    так как все слова уникальны
 * 2) много одинаковых слов, но много документов -> O(n), где n в данном случае  - количество документов.
 *    Например, 10000 документов, состоящих из одинаковых слов.
 * Дополнительно мы храним запрос в хеш-таблице, но это уже зависит от размера запроса, даже если мы его целиком копируем, то он все равно
 * обычно короче документа и если ему нужно памяти r, то O(r)~O(2r).
 * List я использую, чтобы хранить результат решения задачи и сами документы, ведь предполагается, что если мы ищем документы, то их тоже
 * желательно хранить.
 * 
 */
